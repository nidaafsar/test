 ...
 */
@Controller
public class CODBRManageController {
...
//    public final static String closeDialogView = "/module/htmlformentry/closeDialog";
    ...
    public final static String FORM_PATH = "/module/codbr/htmlFormEntry";
    ...
    public FormEntrySession getFormEntrySession(HttpServletRequest request,
    	...
    	/*MySQLJDBCConnection myjdbc = new MySQLJDBCConnection();
    	myjdbc.readDataBase();
*/    	
       ...
       //patientId = id.getPatientId();
        //formId = 4; // <-----------------------this is where you need to mention the FORM ID or HTML Form id in below variable
        //htmlFormId = 5; //<--------------------|
...
//        }
       // String modeParam = request.getParameter("mode"); //MAIMOONA: MODE WOULD NEVER BE EDIT.. WE ARE MAKING FRESH ENTRIES
       ...
       // }
        ...
        if (StringUtils.hasText(request.getParameter("encounterId"))) { //MAIMOONA: FORM WOULD NEVER HAVE ANY ENCOUNTER ID
        ...
        if (mode != Mode.ENTER && patient == null)
            throw new IllegalArgumentException("No patient with patientId=" + patientId);
        FormEntrySession session = null;
        /*if (mode == Mode.ENTER && patient == null) { WOULD NEVER ACCEPT ANY FORM WITHOUT PATIENT
            patient = new Patient();
        }*/
        if (encounter != null) {
